...
import time
import json
import message_objects.payload_data_objects.payload_json_data as JSON_packet
...
if "__main__" == __name__:
    ...
    while running:
        ...
        if type == "login":
            users.append( struct["name"] )
            return_data = { "success": True }

            packet = JSON_packet.PayloadJsonData()
            packet.set_structure( return_data )
            send_payload_object_queue.put( packet )
